...
SLT_feedback_graph_normal_curve <- function(perc_correct, x_min = 40, x_max = 160, x_mean = 100, x_sd = 15) {
  q <-
...
#' \dontrun{
#' SLT_demo(feedback = SLT_feedback_with_score())}
SLT_feedback_with_graph <- function(dict = SLT::SLT_dict) {
  psychTestR::new_timeline(
    psychTestR::reactive_page(function(state, ...) {
      ...
      #browser()
      if (is.null(results$SLT$score)) {
        num_correct <- sum(attr(results$SLT$ability, "metadata")$results$score)
        num_question <- results$SLT$num_items
        perc_correct <- (results$SLT$ability+4)/8
      }
      else {
        num_correct <- round(results$SLT$score * results$SLT$num_questions)
      ...
      norm_plot <- SLT_feedback_graph_normal_curve(perc_correct)
      psychTestR::page(
        ui = shiny::div(
          shiny::p(text_finish),
          shiny::p(norm_plot),
